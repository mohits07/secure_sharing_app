...
var url =  "mongodb://localhost:27017/fileshare";
...
//Responsible to set HTML pages as views. views folder is mandatory
...
app.post(
  ...
  [
    ...
    check("name")
      .isLength({ max: 80 })
      .withMessage("Name max 80 char"),
    check("username")
      .isLength({ min: 6 })
      .withMessage("Username must be at least 6 characters long"),
    check("username")
      .isLength({ max: 16 })
      .withMessage("Username can be at most 16 characters long"),
    check("username")
      .isAscii()
      .withMessage("Username can only contain ASCII characters"),
    check("username")
    ...
    check("password")
      .isLength({ max: 20 })
      .withMessage("Password can be atmost 20 characters long"),
    check(
      "password",
          ...
          function (err, addedUser) {
            ...
            if (err) {
              ...
              passport.authenticate("local")(req, res, function () {
                req.flash(
                  "success",
                  "Welcome to SPPP, " + addedUser.actualName + "!"
                );
                res.redirect("/home");
              });
            }
  ...
  uploadS3(req, res, function (err) {
    if (err) {
      if (err.code === "LIMIT_FILE_SIZE") {
        req.flash(
          "error",
          "File Size Too Large. Current Item Size Limit is " +
            fileLimitUpload +
            " MB."
        );
    ...
    var capacity =
      req.user.currentUsage + Math.ceil(req.file.size / Math.pow(1024, 2));
    if (capacity > req.user.userLimit) {
      deleteFromS3(req.file.key);
      req.flash(
        "error",
        "Cannot upload this item as total storage for " +
          req.user.username +
          " is exceeding user limit of " +
          req.user.userLimit +
          " MB."
      );
      res.redirect("/home/new");
        ...
        Item.create(newItem, function (err, addedItem) {
          if (err) {
            ...
            if (!flag) {
              req.flash(
                "success",
                "Item added, however one or more groups exceeded limit."
              );
            } else {
    ...
    .exec(function (err, foundItem) {
      if (err || !foundItem) {
        ...
        Item.findByIdAndUpdate(
          req.params.id,
          foundItem,
          function (err, updatedWithLastAccessItem) {
            if (err) {
              console.log(err);
            } else {
            }
      ...
      if (err || !foundItem) {
        ...
        User.findById(req.user._id, function (err, foundUser) {
          if (err) {
            ...
            capacity =
              fileSize + Number(Math.ceil(req.file.size / Math.pow(1024, 2)));
            if (capacity > req.user.userLimit) {
              deleteFromS3(req.file.key);
              req.flash(
                "error",
...
app.post("/groups", isLoggedIn, function (req, res) {
  ...
  Group.create(newGroup, function (err, addedGroup) {
    if (err) {
      ...
      req.flash(
        "success",
        "Group successfully added. Please wait for an admin to accept the group."
      );
      res.redirect("/groups");
    }
  });
...
app.post("/admin", isAdmin, function (req, res) {
  ...
  req.flash(
    "success",
    "Item Size Limit successfully changed to " + fileLimitUpload + " MB."
  );
  res.redirect("/admin");
});
...
app.put("/admin/groups/:id", isAdmin, function (req, res) {
  ...
  Group.findByIdAndUpdate(
    req.params.id,
    { $set: { isActive: isActiveStatus, groupLimit: groupLimitSize } },
